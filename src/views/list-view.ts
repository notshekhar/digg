...
export function renderList(store: ClusterStore, state: ListViewState, width: number, bodyHeight: number): string[] {
    const lines = [pad(header(store), width), ui.rule("─".repeat(width)), ...store.table.render(width, bodyHeight)];
...
function header(store: ClusterStore): string {
...
function footer(store: ClusterStore, state: ListViewState): string {
    ...
    const sel = state.mouseEnabled ? ui.dim("  · m to select text") : ui.accent("  · select mode (m)");
    const keys = "enter open · n ns · c ctx · / filter · y yaml · d describe · l logs · x del · R refresh · esc home · ctrl+c quit";
    return `  ${ui.accent("[:] resources")}  ${ui.footer(keys)}${sel}`;
}
